...
- **Dépôt chez l'agent pour des parties bonus ou des points, avec frais de service** : le joueur peut déposer entre `MIN_DEPOSIT_HTG` et `MAX_DEPOSIT_HTG` (autant de fois qu'il veut) pour acheter, au choix, des parties bonus (`HTG_PER_BONUS_PLAY` HTG = 1 partie) ou des points directement (`POINTS_PER_HTG_PURCHASE` points par HTG net). Un frais de service de `DEPOSIT_FEE_PERCENT` % (5% par défaut) est prélevé sur le montant avant de calculer ce qui est accordé — le joueur voit clairement le montant versé, le frais et le résultat obtenu avant de confirmer. **Achat à sens unique** : cet argent n'est jamais reconvertible en points retirables — les parties bonus ne servent qu'à débloquer des parties au-delà de la limite gratuite, et les points achetés rejoignent le solde de jeu mais restent en permanence exclus de ce qui peut être retiré en espèces (`users.non_cashable_points`) — voir "Pourquoi les dépôts ne sont pas retirables" ci-dessous.
...
- **Défi du jour** : obtenir au moins 80% de bonnes réponses dans une seule partie (quiz ou sprint, peu importe lequel) rapporte 50 points bonus, une fois par jour civil — basé sur la performance, pas sur la simple présence. Affiché sur l'accueil (statut du jour) et confirmé sur l'écran de résultat de partie dès qu'il vient d'être relevé.
- **Panneau publicitaire** : image au format portrait (9:16) affichée en surimpression une fois par session au joueur et à l'agent, fermable via un bouton (x) — purement visuel, aucun lien cliquable. Configuré depuis `/admin.html` → Réglages, remplaçable à tout moment ; sans image définie, aucun panneau ne s'affiche.
- **Accueil "vitrine"** : les deux jeux sont désormais présentés comme deux cartes distinctes avec badge lumineux (au lieu de simples tuiles), et un bandeau apparaît sur l'accueil quand un thème saisonnier est actif — voir "Accueil vitrine et défi du jour" ci-dessous pour le détail.
...
- Le serveur détermine **lui-même** si le temps annoncé (25s) est réellement dépassé avant d'appliquer la pénalité de défaite — un signal du client prétendant à tort qu'il y a eu expiration est ignoré si le serveur mesure que ce n'est pas le cas, pour ne jamais pénaliser un joueur à tort.
...
Un dépôt chez l'agent achète des parties bonus ou des points, jamais des points retirables — c'est un choix délibéré, pas une limitation technique. Si l'argent déposé pouvait ensuite ressortir en espèces (même indirectement, via des points gagnés en jouant avec cet argent), Konkou ressemblerait à un système de mise/pari plutôt qu'à une app de récompenses basée sur la performance, ce qui changerait sa qualification légale (voir "Important sur le modèle choisi" en haut de ce document) et l'exposerait à des règles bien plus strictes. Les circuits — points gagnés en jouant (retirables) et argent déposé chez l'agent, que ce soit sous forme de parties bonus ou de points (non retirables) — sont donc gardés strictement séparés dans la base de données.

**Achat direct de points (juillet 2026).** En plus des parties bonus, un joueur peut désormais acheter des points directement chez l'agent (`POINTS_PER_HTG_PURCHASE` points par HTG net, même frais de service `DEPOSIT_FEE_PERCENT` que les parties bonus) — pratique pour un joueur qui veut miser ou continuer à jouer sans passer par le compte à points par partie. Ces points rejoignent le même solde `users.points` que les points gagnés en jouant (donc utilisables pour jouer, miser, etc.), mais leur montant est aussi ajouté à `users.non_cashable_points`, une colonne séparée qui ne bouge jamais quand des points sont gagnés en jouant. Un retrait (`routes/wallet.js`, `postCashout`) est plafonné à `points - non_cashable_points` (jamais au solde total) — donc peu importe comment le solde évolue ensuite (mise, dépense, partie perdue...), la portion achetée reste par construction exclue de ce qui peut ressortir en espèces. Le Portefeuille affiche cette répartition (points retirables vs achetés) dès qu'un joueur a des points non retirables en solde.
...
**Achat de points chez l'agent, en plus des parties bonus (juillet 2026).** Le formulaire de dépôt (Portefeuille) propose désormais un choix "Parties bonus" / "Points" avant de saisir le montant en HTG — même code agent, même frais de service (`DEPOSIT_FEE_PERCENT`), même flux de confirmation (agent d'abord, admin en secours). Un dépôt "points" crédite `POINTS_PER_HTG_PURCHASE` points par HTG net directement sur le solde de jeu du joueur, utilisables pour jouer ou miser — mais jamais retirables : voir "Pourquoi les dépôts ne sont pas retirables" plus haut pour l'implémentation (`users.non_cashable_points`) qui garantit cette séparation même après que le solde a bougé (mise, dépense, partie perdue). Le Portefeuille affiche la répartition retirable/non retirable dès qu'un joueur a des points achetés en solde, et le formulaire de retrait refuse toute demande qui dépasserait la portion retirable. Testé de bout en bout (script Node ciblant les fonctions de route directement sur une base de test) : validation du format téléphone, bonus de bienvenue, non-répétition du bonus après suppression/réinscription, `timeLimitSeconds` à 30, formule de mise aux deux extrêmes (0.25 et 1.10) — voir aussi les tests dédiés à cette fonctionnalité : achat de points créditant bien `points` et `non_cashable_points` à l'identique, un retrait au-delà de la portion retirable rejeté, un retrait dans la portion retirable accepté normalement.

**Chronomètre resserré à 25 secondes (juillet 2026).** `TRIVIA_TIME_LIMIT_SECONDS` et `PUZZLE_TIME_LIMIT_SECONDS` passent de 30 à 25 dans `backend/routes/games.js` — troisième resserrement après 60s → 45s → 30s. Comportement inchangé sinon : même pénalité de défaite en cas d'expiration (0 point, mise à -50%), même vérification côté serveur indépendante du client.

**Accueil vitrine et défi du jour (juillet 2026).** Deux ajouts en parallèle, inspirés d'une maquette de jeu mobile envoyée en référence, adaptés au style et aux contraintes de Konkou (zéro-dépendance, pas d'outil de génération d'images — donc un habillage en CSS pur plutôt que des illustrations personnalisées) :
- **Défi du jour** : obtenir au moins `DAILY_CHALLENGE_PERCENT` (80% par défaut, `backend/routes/games.js`) de bonnes réponses dans une seule partie (quiz ou sprint, peu importe lequel) rapporte `DAILY_CHALLENGE_REWARD_POINTS` (50 pts) une seule fois par jour civil — la contrainte `UNIQUE(user_id, claim_date)` sur la nouvelle table `daily_challenge_claims` empêche tout double crédit, même en cas de parties qualifiantes répétées le même jour. Volontairement basé sur la performance (comme la mise et le classement) plutôt que la simple présence. Le statut du jour (relevé ou non, récompense) est exposé par `GET /api/profile` et affiché en carte sur l'accueil ; l'écran de résultat de partie confirme quand le défi vient tout juste d'être relevé.
- **Vitrine visuelle de l'accueil** : les deux jeux passent de simples tuiles à des cartes `.game-panel` avec badge lumineux, dans un esprit "futuriste" sobre (dégradés, lueur cyan `--accent-glow`, CSS pur — aucune image ni police externe ajoutée).

Ce remaniement reste volontairement scopé à l'écran d'accueil (tuiles de jeu, nouvelles cartes) plutôt qu'à toute l'app, pour limiter le risque de régression sur les écrans existants (formulaires, portefeuille, classement) qui gardent leur style `.tile`/`.card` habituel.

Testé de bout en bout (script Node ciblant les routes directement sur une base de test) : statut initial non relevé, un score sous le seuil (0/5) qui ne déclenche rien, un score au-dessus du seuil (7/8 = 87.5%) qui crédite exactement 50 points et enregistre la transaction, le statut qui passe à "relevé" ensuite, et une seconde partie qualifiante le même jour qui ne crédite pas une deuxième fois (une seule transaction `daily_challenge_reward` au total).

**Ajustements après premier retour sur la vitrine (juillet 2026).** Trois changements en parallèle :
- **Style "glow" étendu au reste de l'app** : les cartes principales des onglets Classement, Portefeuille (Solde, Devenir VIP) et Profil reprennent désormais la bordure/lueur cyan `.glow-card` déjà utilisée sur l'accueil, plus un léger effet de lueur sur l'icône de l'onglet actif dans la barre du bas — pour que l'app entière lise comme un ensemble cohérent plutôt qu'un accueil retouché isolément.
- **Défi du jour cliquable** : la carte n'était affichée qu'à titre informatif ; elle est maintenant un vrai bouton tant que le défi n'est pas relevé (mène à un choix Quiz/Sprint dédié, qui lance ensuite le jeu normalement). Une fois relevé, la carte redevient un simple encart non cliquable et affiche le nom et le code de parrainage du joueur qui l'a relevé, à la place du texte "pas encore relevé".
- **Bannière de thème saisonnier retirée côté joueur** : le premier essai (bannière "Thème X actif" sur l'accueil) a été retiré après retour — seul l'admin voit/choisit le thème actif, depuis `/admin.html` → Réglages (le thème continue de s'appliquer normalement pour tout le monde, seule l'annonce visuelle sur l'accueil disparaît).

**Couleur bleu foncé personnalisable (juillet 2026).** Nouveau réglage admin (`/admin.html` → Réglages → "Couleur bleu foncé personnalisée"), même principe que la couleur de fond déjà personnalisable : une seule couleur à choisir, qui remplace `--blue` (barre du haut, badges des jeux sur l'accueil) pour n'importe quel thème actif. Le second ton du dégradé (`--blue-2`) est calculé automatiquement en assombrissant cette couleur (fonction `darkenHex()`, dupliquée dans `app.js`/`admin.js` faute de module partagé côté frontend), pour ne pas demander à l'admin de choisir/assortir deux couleurs. Stocké dans `settings` (`app_blue_color`), exposé par `GET /api/theme` comme `bgColor` l'est déjà. Testé : couleur invalide rejetée, couleur valide appliquée et reflétée immédiatement par `GET /api/theme`, réinitialisation qui revient au bleu du thème actif.

**Panneau publicitaire et texte "Agents" clarifié (juillet 2026).** Deux changements :
- Le texte d'accueil et l'étape 3 du "Comment ça marche" ("...chez notre agent") deviennent "...chez l'un de nos Agents sur tout le territoire national", pour refléter le réseau d'agents décentralisé plutôt qu'un point unique.
- **Nouveau panneau publicitaire** (`/admin.html` → Réglages → "Panneau publicitaire") : une image au format portrait (9:16 recommandé), affichée en surimpression légèrement rétrécie par rapport à l'écran (pas en plein écran), une fois par session, au joueur ET à l'agent — fermable via un bouton (x), purement informatif (aucun lien cliquable, aucun suivi de clic). Sert à promouvoir un avantage de l'app (VIP, parrainage...) ou une entreprise tierce, remplaçable par l'admin à tout moment. Techniquement : `backend/routes/ads.js` (nouveau, réutilise `saveUploadedImage` exportée de `routes/theme.js`), stocké dans `settings` (`app_ad_image`), exposé publiquement par `GET /api/ad` comme le thème (`GET /api/theme`) ; `renderAdOverlayIfNeeded()` dans `app.js` l'affiche au plus une fois par chargement de page (pas de persistance entre sessions — un nouveau chargement de page peut le montrer à nouveau). Testé : upload accepté et reflété immédiatement par `GET /api/ad`, format invalide rejeté, retrait qui vide bien le panneau.

**Code Agent complet (5 lettres + 6 chiffres) et Rapport global des commissions (juillet 2026).** Deux changements liés à la gestion des agents côté Admin :
- **Code Agent affiché en un seul jeton** : le numéro séquentiel de l'agent (`formatAgentNumber`, `backend/routes/agents.js`) passe de 5 à 6 chiffres, et une nouvelle fonction `fullAgentCode(agentCode, agentId)` concatène le code lettres (3 du nom + 2 du prénom, ex. `LEDMA`) et ce numéro en un seul "Code Agent" (ex. `LEDMA000042`). Ce code combiné, accompagné du nom/prénom et de l'adresse, s'affiche désormais partout où un agent apparaît côté Joueur (sélecteur d'agent sur le formulaire de dépôt/retrait, écran de candidature en attente/rejetée, tableau de bord agent) et côté Admin (candidatures Agents, Renflouements).
- **Nouveau "Rapport global"** (`/admin.html` → Agents → onglet "📋 Rapport global") : un tableau listant tous les agents du système (actifs, en attente ou rejetés) avec leur Code Agent, Nom et Prénom, Numéro de téléphone, Commissions générées et Retraits effectués, filtrable par période (`from`/`to`, dates facultatives — sans filtre, tout l'historique) via la nouvelle route `GET /api/admin/agents/report`. Seuls les retraits **payés** (`cashouts.status = 'paid'`) comptent, sur leur date de traitement (`processed_at`) — un retrait encore en attente ou rejeté ne génère aucune commission. Un bouton "🖨️ Imprimer" déclenche l'impression du navigateur ; une règle `@media print` dédiée (`frontend/styles.css`) masque tout le reste de l'interface admin (menu, filtres, boutons) pour n'imprimer que le tableau du rapport, prêt pour le suivi des paiements de commission à chaque agent.

Testé (script Node ciblant les fonctions de route directement sur une base de test) : `formatAgentNumber`/`fullAgentCode` produisent bien un jeton à 6 chiffres puis 5+6 caractères ; un agent avec des retraits payés dans et hors de la période choisie n'agrège que ceux dans la période (commission et compte corrects) ; un agent sans aucun retrait renvoie une ligne à zéro plutôt qu'une erreur ; les totaux du rapport correspondent à la somme des lignes ; `listAgentApplications`/`listActiveAgents` exposent bien le nouveau Code Agent combiné.
