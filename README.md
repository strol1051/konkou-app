...
N'importe quel joueur peut devenir agent directement dans l'app (Profil → "Espace Agent") :

1. **Candidature** : nom, prénom, date de naissance (l'app vérifie que l'agent a 18 ans ou plus), type de pièce d'identité (CIN, passeport ou permis de conduire) et son numéro — **obligatoire**. L'app génère un **code agent** à partir de 3 lettres du nom + 2 lettres du prénom (ex. Pierre Louis → `PIELO`) — un suffixe numérique est ajouté en cas de collision avec un code déjà pris.
...
La suppression est **refusée** si l'une de ces conditions est vraie :
- le compte a un solde de points non nul (le joueur doit d'abord tout retirer) ;
...
3. **Recruter et approuver vos premiers agents** (Profil → Espace Agent dans l'app, puis onglet *Agents* de `/admin.html`) — sans agent actif, aucun joueur ne peut déposer ni retirer, puisque les deux exigent désormais un code agent valide.
...
**Suppression de compte, joueur et admin (juillet 2026).** Nouveau bouton "Supprimer mon compte" (Profil, avec confirmation par mot de passe) et nouvel onglet *Comptes* dans `/admin.html` (recherche par téléphone, suppression admin). Les deux chemins partagent exactement les mêmes garde-fous — voir "Suppression de compte" plus haut : refusé si solde de points non nul, retrait/dépôt en attente, ou rôle agent actif. Testé de bout en bout : suppression bloquée dans chacun des trois cas de garde-fou, mauvais mot de passe rejeté côté joueur, suppression réussie une fois le solde à zéro et aucune obligation en attente, compte introuvable après suppression (login et recherche admin échouent tous les deux proprement).
