...
- **Vitrine visuelle de l'accueil** : les deux jeux passent de simples tuiles à des cartes `.game-panel` avec badge lumineux, dans un esprit "futuriste" sobre (dégradés, lueur cyan `--accent-glow`, CSS pur — aucune image ni police externe ajoutée). Une bannière `.glow-card` apparaît en haut de l'accueil quand un thème saisonnier autre que "Défaut" est actif (`activeThemeKey` côté client, alimenté par `GET /api/theme` déjà existant), pour le rendre plus visible qu'un simple changement de couleurs. La carte "Défi du jour" reprend le même langage visuel (bordure/lueur), et bascule au vert une fois relevée.
